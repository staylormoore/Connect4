...
class Board:
    ...
    def updateBoard(self, col, PL): # changes the board with each click of a bottom button
        pos = 5
        num = 5
        if self.IsFull(col) == False: # if the col they selected is not full
            ...
            # sets the empty slot to the corrisponding number for the player


    def Winner(self):  # checks if there is a winner and returns true if so
        # check horizontal for win
        for c in range(4): # goes through 4 columns
            for r in range(6): # goes through 6 rows
                piece = self.board[r][c]
                if piece !=0 and self.board[r][c + 1] == piece and self.board[r][c + 2] == piece and self.board[r][c + 3] == piece:
                    # if there is 4 in a row horizontally
                    return True

        # Check vertical locations for win
        for c in range(7): # goes through 7 columns
            for r in range(3): # goes through 3 rows
                piece = self.board[r][c]
                if piece !=0 and self.board[r + 1][c] == piece and self.board[r + 2][c] == piece and self.board[r + 3][c] == piece:
                    # if there is 4 in a row vertically
                    return True
        ...
        for c in range(4): # goes through 4 columns
            for r in range(3): # goes through 3 rows
                ...
                if piece !=0 and self.board[r + 1][c + 1] == piece and self.board[r + 2][c + 2] == piece and self.board[r + 3][c + 3] == piece:
                    # if there is 4 in a row diagonally with a negative slope
                    return True

        # Check positively sloped diagonals
        for c in range(4): # goes through 4 columns
            for r in range(3): # goes through 3 columns
                piece = self.board[r][c + 3]
                if piece !=0 and self.board[r + 1][c + 2] == piece and self.board[r + 2][c + 1] == piece and self.board[r + 3][c] == piece:
                    # if there is 4 in a row diagonally with a positive slope
                    return True
        return False
